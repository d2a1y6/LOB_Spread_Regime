"""
程序名称：07_ResNet_Attention_GRU_GPU.py
功能描述：基于 ResNet-Attention-GRU 的混合神经网络模型

1. 模型原理 (Hybrid Architecture)：
   [Input Sequence] (Batch, 20, 13)
          ⬇
   [TimeDistributed ResNet] 对每个时间步独立进行深层特征提取 (解决特征交互问题)
          ⬇
   [High-Level Features] (Batch, 20, 64)
          ⬇
   [Bi-GRU Layer] 双向捕捉时序演变 (解决时间依赖问题)
          ⬇
   [Attention Mechanism] 自动加权关键时间步 (解决长序列遗忘/噪音问题)
          ⬇
   [Classifier] 全连接层 -> Logits

2. 核心特性：
   - 结合了 ResNet 的非线性特征提取能力与 GRU 的时序建模能力。
   - 针对 RTX 4090 进行了全链路优化 (Batch=16384, Pin_Memory)。
"""
...
# 模型超参数
SEQ_LEN = 20           # 时间窗口
LEARNING_RATE = 1e-3   # 混合模型参数较多，LR 适中
EPOCHS = 150
PATIENCE = 20
DROPOUT_RATE = 0.3
WEIGHT_DECAY = 1e-4
...
if torch.cuda.is_available():
    ...
    BATCH_SIZE = 12288      # 混合模型显存占用较大，稍微调小 Batch
    NUM_WORKERS = 8
    PIN_MEMORY = True
...
class TrainingVisualizer:
    def __init__(self):
        self.epochs, self.losses, self.val_aucs, self.val_accs = [], [], [], []
    ...
    def save_plot(self, filename):
        ...
        plt.title('ResNet-Attention-GRU Training Metrics')
        plt.grid(True, alpha=0.3); plt.tight_layout()
        plt.savefig(filename); plt.close()

# ==============================================================================
# 2. 混合模型定义 (ResNet Block + GRU + Attention)
# ==============================================================================
...
# ==============================================================================
def run_pipeline():
    print(f"启动混合模型 (ResNet + Attn-GRU) 训练...")
    start_time = time.time()
    model_path = 'models/best_hybrid_resnet_gru.pth'
    ...
    # --- 模型初始化 ---
    # ResNet特征维度=64, GRU隐层=128
    model = ResNetAttentionGRU(input_dim=len(cols), resnet_dim=64, gru_dim=128).to(DEVICE)
    ...
    for epoch in range(EPOCHS):
        ...
        if val_auc > best_auc:
            ...
            if patience_cnt >= PATIENCE:
                print(f"早停触发。Best AUC: {best_auc:.4f}")
                break

    visualizer.save_plot("models/training_curve_hybrid.png")
    ...
    print("="*50)
    print(f"Hybrid Model Test AUC : {auc:.4f}")
    print(f"Hybrid Model Test Acc : {accuracy_score(all_targets, preds):.4f}")
